...
type SubCommandFactory = fn(Arc<dyn ShellExtensionHost>, AppRoot) -> IExplorerCommand;
...
impl IEnumExplorerCommand_Impl for SubCommands_Impl {
    ...
    fn Next(
        ...
        while remaining > 0 && *current < SUB_COMMAND_FACTORIES.len() {
            ...
            let command = factory(self.host.clone(), self.app_root.clone());
            unsafe {
...
macro_rules! sub_command_factory {
    ($name:ident, $handler:ident) => {
        fn $name(host: Arc<dyn ShellExtensionHost>, app_root: AppRoot) -> IExplorerCommand {
            $handler::new(host, app_root).into()
        }
    };
}
