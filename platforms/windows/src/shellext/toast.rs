use crate::shell_host::{
    ConflictResolutionAction, ConflictResolutionRequest, ShellExtensionHost,
};
...
impl ToastActivator {
    ...
    fn handle_resolve_action(&self, inputs: &[ToastInputData], params: &HashMap<String, String>) {
        ...
        if let Some(first_input) = inputs.first() {
            ...
            let conflict_action = match first_input.value.as_str() {
                "overwrite_remote" => ConflictResolutionAction::OverwriteRemote,
                "save_as_new" => ConflictResolutionAction::SaveAsNew,
                _ => ConflictResolutionAction::KeepRemote,
            };
            if let Err(e) = self.host.resolve_conflict(ConflictResolutionRequest {
    ...
    fn handle_settings_action(&self, params: &HashMap<String, String>) {
        ...
        if let Err(e) = self.host.open_settings_window() {
...
impl IClassFactory_Impl for ToastActivatorFactory_Impl {
    fn CreateInstance(
        ...
        let handler = ToastActivator::new(self.host.clone());
